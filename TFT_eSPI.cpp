...
const uint8_t  PEmap[63][6]  PROGMEM = {
...
#if defined (TFT_BL) && !defined(RP2040_PIO_INTERFACE)
    pinMode(TFT_BL, OUTPUT);
    digitalWrite(TFT_BL, HIGH); // Data/Command high = data mode
#endif
...
void TFT_eSPI::PutCharEN(unsigned char c)
{
  ...
  if( (c < firstchar) || (c > lastchar) || (fontSize == 0)) 
	...
	// line adjust
  // TFTWidth=LCD_WIDTH-1;
	if(((int)cursorX + charwidth * FontXScale) > TFTWidth)
	{
...
void TFT_eSPI::PutCharPE(unsigned char c)
{
		unsigned char 			byte 					= 0;
		unsigned char 			bitoffset 		= 0;
		unsigned char 			maske 				= 0;
      ...
      // TFTWidth=LCD_WIDTH-1;
			cursorY = cursorY   + (unsigned int)FontHeight * FontYScale; 
			cursorX = TFTWidth - (unsigned int)charwidth * FontXScale;
		}
...
unsigned int TFT_eSPI::CalcTextWidthEN(char *str)
{
...
unsigned int TFT_eSPI::CalcTextWidthPE(char *str)
{
...
unsigned int TFT_eSPI::CalcTextWidth(char *Text)
{
...
unsigned int TFT_eSPI::CalcTextHeight(void)
{
...
unsigned char   TFT_eSPI:: utf8ascii(unsigned char ascii) {

    if ( ascii < (unsigned char)128 )   // Standard ASCII-set 0..0x7F handling
    {   
	  c1=0;
      return( ascii );
    }
...
case (unsigned char)0xd8:
        if(ascii >= (unsigned char)0xa0 && ascii <= (unsigned char)0xb6)
        {
        return (ascii + 0x20);  
        }

        if(ascii>=(unsigned char)0xb7 && ascii<=(unsigned char)0xba)
        {
        	return (ascii + 0x21);
        }
        break;
case (unsigned char)0xd9:
        if(ascii>=(unsigned char)0x80 && ascii<=(unsigned char)0x83)
        {
        	return (ascii + 0x5c); 
        }

        if(ascii==(unsigned char)0x84)
        {
        	return (ascii + 0x5d);
        }

        if(ascii>=(unsigned char)0x85 && ascii<=(unsigned char)0x88)
        {
        	return (ascii + 0x5e);
        }

        if(ascii==(unsigned char)0x89 || ascii==(unsigned char)0x8a)
        {
        	return (ascii + 0x63); 
        }

        if(ascii==(unsigned char)0xbe)
        {
        	return 0x81;
        }
        break;
case (unsigned char)0xda:
        if(ascii==(unsigned char)0x86)
        {
        	return 0x8d;
        }
        if(ascii==(unsigned char)0x98)//zhe
        {
        	return 0x8e;
        }

        if(ascii==(unsigned char)0xaf)
        {
        	return 0x90;
        }
        if(ascii==(unsigned char)0xa9)//ke
          {
          	return 0xdf;
          }
				break;	
case (unsigned char)0xdb:
        if(ascii==(unsigned char)0x8c)
        {
        	return 0xed;
        }
        break;
...
void TFT_eSPI:: PutsPE(char *str0)
{	
		unsigned char 	curnLet;
		unsigned char 	Pstat, stat, Nstat;
		
		unsigned char		backFontSpace = FontSpaceX;
		unsigned char 						strTemp[50];
...
//-------------------------------------------------
//**** select font
void TFT_eSPI:: SetFont(const unsigned char *pointer)
{
	FontPointer 			= pointer;
...
void TFT_eSPI:: SetLetter(Lt_Mode L)
{
		peLETTER = L;

		if(L == P_LETTER){
			 if(p_rotation ==1 || p_rotation ==3) {TFTWidth=LCD_HEIGHT-1;TFTHeight=LCD_WIDTH-1;cursorX = TFTWidth;}else if(p_rotation ==0 || p_rotation ==2){TFTWidth=LCD_WIDTH-1;TFTHeight=LCD_HEIGHT-1;cursorX = TFTWidth;};

}else{
  if(p_rotation ==1 || p_rotation ==3) {TFTWidth=LCD_HEIGHT-1;TFTHeight=LCD_WIDTH-1;cursorX = 0;}else if(p_rotation ==0 || p_rotation ==2){TFTWidth=LCD_WIDTH-1;TFTHeight=LCD_HEIGHT-1;cursorX = 0; };
  }
		
			
}	//*SetLetter

void TFT_eSPI:: SetLine(unsigned char line,unsigned char column)
{
		if(peLETTER == P_LETTER){
			if(p_rotation ==1 || p_rotation ==3) 
			{
				cursorX =TFTWidth - (unsigned int)FontWidth  * FontXScale * column;
				}else if(p_rotation ==0 || p_rotation ==2){
					cursorX =TFTWidth - (unsigned int)FontWidth  * FontXScale * column;
					};
		
		}else{cursorX =(unsigned int)FontWidth  * FontXScale * column;};
...
//**** draw char
void TFT_eSPI:: PutChar(unsigned char c)
{
	  if(peLETTER == P_LETTER)
			PutCharPE(c);
		else
...
unsigned char TFT_eSPI:: Bit_Is_Set(unsigned char Byte ,unsigned char Mask)
{
	if(Byte&(0x01<<Mask))
		return 1;
	else 
...
void TFT_eSPI::drawString(unsigned char line, unsigned char column,char *str,unsigned int Fg,unsigned int Bg)
{
FgColor 				= Fg;			
BkColor 				= Bg;
SetLine(line,column);
...
void TFT_eSPI::LcdFontXScale(unsigned char n)
{
FontXScale = n;
}

void TFT_eSPI::LcdFontYScale(unsigned char n)
{
FontYScale = n;
}

void TFT_eSPI::SetCursor(unsigned char x ,unsigned char y )
{
cursorX = x;cursorY = y;
}

void TFT_eSPI::SetFgColor(unsigned char n)
{
FgColor = n;
}
void TFT_eSPI::SetBkColor(unsigned char n)
{
BkColor = n;
}
