...
#define PutPixel(x, y, clr)					drawPixel( x , y , clr )
...
class TFT_eSPI : public Print { friend class TFT_eSprite; // Sprite class has access to protected members
 ...
 public:
  ...
  virtual void     drawPixel(int32_t x, int32_t y, uint32_t color),
   ...
   SetFont(const uint8_t* pointer),
   SetLetter(Lt_Mode L),
   SetLine(unsigned char line,unsigned char column),
   PutChar(unsigned char c),
   Puts(char *Text),
   PutsPE(char *str0),
   drawString(unsigned char line, unsigned char column,char *str,unsigned int Fg,unsigned int Bg),
   LcdFontXScale(unsigned char n),
   LcdFontYScale(unsigned char n),
   SetCursor(unsigned char x ,unsigned char y ),
   SetFgColor(unsigned char n),
   SetBkColor(unsigned char n),
   PutCharEN(unsigned char c),
	 PutCharPE(unsigned char c);
  ...
  virtual int16_t  drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font),
                   ...
                   height(void),
                   width(void);
  virtual    unsigned int CalcTextWidthEN(char *str);
  virtual 	unsigned int CalcTextWidthPE(char *str);
  virtual 	unsigned int CalcTextWidth(char *Text);
  virtual 	unsigned int CalcTextHeight(void);
  virtual 	unsigned char Bit_Is_Set(unsigned char Byte ,unsigned char Mask);
  virtual 	unsigned char utf8ascii(unsigned char ascii); 
...
unsigned int 						FgColor 				= BLACK;			
unsigned int 						BkColor 				= WHITE;			
...
unsigned int 							TFTWidth;
unsigned int 							TFTHeight;
